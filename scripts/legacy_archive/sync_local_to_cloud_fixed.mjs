...
async function runSync() {
    console.log("🚀 Iniciando Sincronização LOCAL -> NUVEM (Schema Compatível)...\n");

    // Tentar encontrar o banco local
    const possiblePaths = [
    ...
    try {
        // 1. Usuarios - Mapear apenas campos compatíveis
        ...
        // 2. Vendedores - Mapear apenas campos compatíveis
        ...
        // 3. Portais - Mapear apenas campos compatíveis
        ...
        if (portais.length > 0) {
            const portaisMapped = portais.map(p => ({
                nome: p.nome,
                loja_id: p.loja_id,
                ativo: !!p.ativo
            }));

            const { error } = await supabase.from('portais').upsert(portaisMapped, {
                onConflict: 'nome,loja_id'
            });
